...
export default (Model, ctx, opt, cb) => {
	...
	const make = (newdata) => {
		let slugify = (str) => {
			let from = 'ąàáäâãåæćęèéëêìíïîłńòóöôõøśùúüûñçżź';
			let	to = 'aaaaaaaaceeeeeiiiilnoooooosuuuunczz';
		...
		// fix
		let startAt = options.separator.length;
		...
		Model.find({
			...
			if (err) {
				...
				let max = docs.reduce(function (mx, doc) {
					let docSlug = doc[options.slug];
					let count = 1;
